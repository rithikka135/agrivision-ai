...
app = FastAPI(
    ...
    version="4.2.0",
    description="Enterprise AI diagnostic engine combining vision inference, Grad-CAM, weather ingestion, and dosage math."
)
...
async def serve_ui():
    ...
    if not os.path.exists(html_path):
        # Fallback path if index.html is placed in root
...
async def diagnose_crop(
    ...
    """Primary diagnosis endpoint processing vision model, weather API, chemical dosage, and audit logging."""
    if not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="Invalid file type. Please upload an image.")
    
    image_bytes = await file.read()
    disease, confidence, gradcam_data = classifier.predict(image_bytes)
    
    # Process Grad-CAM Base64 format safely
    ...
    # Low-Confidence / Out-of-Distribution (Non-Crop) Guard
    if "Unclear" in disease or confidence < 55.0:
        ...
        advisory = {
            ...
            "llm_advisory": "The uploaded photo is either not a recognized leaf or is too blurry for an accurate diagnosis. Please re-upload a clear, close-up photograph of the affected plant leaf.",
            "audio_b64": ""
        }
        dosage_info = {
            "status": "No Chemical Required",
            "message": "Diagnosis inconclusive. Please upload a clear leaf image before calculating treatment dosage."
        }
        economic_info = {
            "status": "Calculation Skipped",
            "crop_type": "Unknown",
            "projected_yield_kg": "N/A",
            "estimated_financial_loss": "₹0 (Inconclusive Image)",
            "action_note": "Re-upload a clear photograph of the infected leaf to calculate economic loss risk."
        }
    else:
    ...
    # Log scan metadata into SQLite database
    save_scan_record(
        ...
        lon=longitude,
        acres=field_acres,
        risk=advisory["weather_context"].get("risk_score", "Moderate Risk")
    )
...
async def download_pdf(
    ...
    pdf_bytes = generate_pdf_report(
        ...
        advisory_text=advisory["llm_advisory"],
